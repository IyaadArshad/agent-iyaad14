...
const MessageActionButtons: React.FC<MessageActionButtonsProps> = ({
  ...
  useEffect(() => {
    ...
    if (!hasShownRef.current) {
      ...
      console.debug(`MessageActionButtons: Scheduling visibility for ${messageId} after ${totalDelay}ms`);
    }
    ...
    if (isComplete && !hasShownRef.current && !timerRef.current && showAfterMs === 0) {
      setVisible(true);
      hasShownRef.current = true;
    }
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              ...
              onClick={() => navigator.clipboard.writeText(content).then(() => console.log("Copied:", messageId))}
              disabled={!visible}
